...
Usage:
    python update_gamedata.py -gamever=<version> [-configyaml=config.yaml] [-bindir=bin] [-platform=windows,linux]
...
def load_all_yaml_data(config, bin_dir, gamever, platforms):
    """
    ...
    Returns:
        Dictionary: {func_name: {"library": str, "category": str, platform: yaml_data}}
    """
    yaml_data = {}
...
def update_counterstrikesharp(yaml_data, func_lib_map, platforms, script_dir):
    """
...
def update_cs2fixes(yaml_data, func_lib_map, platforms, script_dir):
    """
...
def update_cs2kz(yaml_data, func_lib_map, platforms, script_dir):
    """
...
def update_swiftlys2(yaml_data, func_lib_map, platforms, script_dir, alias_to_name_map):
    """
    ...
    if os.path.exists(sig_path):
        updated, skipped = update_swiftlys2_signatures(
            yaml_data, func_lib_map, platforms, sig_path, alias_to_name_map
        )
    ...
    if os.path.exists(off_path):
        updated, skipped = update_swiftlys2_offsets(
            yaml_data, func_lib_map, platforms, off_path, alias_to_name_map
        )
        total_updated += updated
        total_skipped += skipped
    else:
...
def update_swiftlys2_signatures(yaml_data, func_lib_map, platforms, sig_path, alias_to_name_map):
    """
...
def update_swiftlys2_offsets(yaml_data, func_lib_map, platforms, off_path, alias_to_name_map):
    """
...
def update_plugify(yaml_data, func_lib_map, platforms, script_dir, alias_to_name_map):
    """
...
def main():
    ...
    # Load all YAML data
    print("\nLoading YAML data...")
    yaml_data = load_all_yaml_data(config, bin_dir, gamever, platforms)
    print(f"Loaded data for {len(yaml_data)} functions")

    # Update each gamedata format
    print("\n" + "=" * 50)
    print("Updating CounterStrikeSharp...")
    css_updated, css_skipped = update_counterstrikesharp(
    ...
    cs2f_updated, cs2f_skipped = update_cs2fixes(
        yaml_data, func_lib_map, platforms, script_dir
    )
    ...
    cs2kz_updated, cs2kz_skipped = update_cs2kz(
        yaml_data, func_lib_map, platforms, script_dir
    )
    ...
    plugify_updated, plugify_skipped = update_plugify(
        yaml_data, func_lib_map, platforms, script_dir, alias_to_name_map
    )
