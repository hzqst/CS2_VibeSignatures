...
def py_eval(
    ...
    try:
        ...
        try:
            ...
            # Only eval top-level expressions (no indentation) to avoid
            # re-executing code inside conditional branches
            lines = code.strip().split("\n")
            if lines:
                last_line_raw = lines[-1]
                # Skip if the line has leading whitespace (inside a block)
                if last_line_raw and not last_line_raw[0].isspace():
