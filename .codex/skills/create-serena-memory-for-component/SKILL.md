...
## 触发语句示例（用于匹配）
- 对 `XXXXX.py` 进行源码级梳理，并将该组件的概述/架构写入 Serena memory：`XXXXX.md`
- 详细分析 `XXXXX` 并将该业务模块的概述/架构以 serena memory 的形式写入 `XXXXX.md`
- 对 `XXXXX` 进行源码级梳理，并将该组件的概述/架构写入 Serena memory：`XXXXX.md`
- 详细分析 `XXXXX` 的架构/业务逻辑，并写入 Serena memory：`XXXXX.md`
