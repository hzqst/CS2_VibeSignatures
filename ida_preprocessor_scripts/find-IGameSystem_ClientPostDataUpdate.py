...
Programmatically determines IGameSystem_ClientPostDataUpdate by:
1. Reading CLoopModeGame_OnPostDataUpdate func_va from its YAML.
2. Finding `lea rdx, sub_XXXXXXX` in that function (= GameSystem_OnClientPostDataUpdate).
3. Finding the virtual call offset inside GameSystem_OnClientPostDataUpdate.
...
async def preprocess_skill(
    ...
    """Resolve IGameSystem_ClientPostDataUpdate from CLoopModeGame_OnPostDataUpdate."""
    _ = skill_name
    ...
    # 1. Read CLoopModeGame_OnPostDataUpdate YAML to get func_va
    src_path = os.path.join(
        new_binary_dir,
        f"CLoopModeGame_OnPostDataUpdate.{platform}.yaml",
    )
    src_data = _read_yaml(src_path)
    if not isinstance(src_data, dict) or not src_data.get("func_va"):
        if debug:
            print("    Preprocess: failed to read CLoopModeGame_OnPostDataUpdate YAML")
        return False
    ...
    # 3. Use py_eval to find GameSystem_OnClientPostDataUpdate and its vfunc offset
    ...
    if not isinstance(vfunc_info, dict):
        if debug:
            print("    Preprocess: failed to determine vfunc offset from CLoopModeGame_OnPostDataUpdate")
        return False
    ...
    # 3b. Rename GameSystem_OnClientPostDataUpdate in IDA
    if game_event_addr:
        try:
            await session.call_tool(
                name="rename",
                arguments={"batch": {"func": {"addr": game_event_addr, "name": "GameSystem_OnClientPostDataUpdate"}}},
            )
        except Exception:
            if debug:
                print("    Preprocess: failed to rename GameSystem_OnClientPostDataUpdate (non-fatal)")
    ...
    if not isinstance(func_info, dict):
        if debug:
            print("    Preprocess: failed to query function info for IGameSystem_ClientPostDataUpdate")
        return False
    ...
    target_name = "IGameSystem_ClientPostDataUpdate"
    payload = {
