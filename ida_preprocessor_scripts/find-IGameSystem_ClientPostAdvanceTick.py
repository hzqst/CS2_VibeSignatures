...
Programmatically determines IGameSystem_ClientPostAdvanceTick by:
1. Reading CLoopModeGame_OnClientPostAdvanceTick func_va from its YAML.
2. Finding `lea rdx, sub_XXXXXXX` in that function (= GameSystem_OnClientPostAdvanceTick).
3. Finding the virtual call offset inside GameSystem_OnClientPostAdvanceTick.
...
async def preprocess_skill(
    ...
    """Resolve IGameSystem_ClientPostAdvanceTick from CLoopModeGame_OnClientPostAdvanceTick."""
    _ = skill_name
    ...
    # 1. Read CLoopModeGame_OnClientPostAdvanceTick YAML to get func_va
    src_path = os.path.join(
        new_binary_dir,
        f"CLoopModeGame_OnClientPostAdvanceTick.{platform}.yaml",
    )
    src_data = _read_yaml(src_path)
    if not isinstance(src_data, dict) or not src_data.get("func_va"):
        if debug:
            print("    Preprocess: failed to read CLoopModeGame_OnClientPostAdvanceTick YAML")
        return False
    ...
    # 3. Use py_eval to find GameSystem_OnClientPostAdvanceTick and its vfunc offset
    py_code = (
    ...
    if not isinstance(vfunc_info, dict):
        if debug:
            print("    Preprocess: failed to determine vfunc offset from CLoopModeGame_OnClientPostAdvanceTick")
        return False
    ...
    # 3b. Rename GameSystem_OnClientPostAdvanceTick in IDA
    if game_event_addr:
        try:
            await session.call_tool(
                name="rename",
                arguments={"batch": {"func": {"addr": game_event_addr, "name": "GameSystem_OnClientPostAdvanceTick"}}},
            )
        except Exception:
            if debug:
                print("    Preprocess: failed to rename GameSystem_OnClientPostAdvanceTick (non-fatal)")
    ...
    if not isinstance(func_info, dict):
        if debug:
            print("    Preprocess: failed to query function info for IGameSystem_ClientPostAdvanceTick")
        return False
    ...
    target_name = "IGameSystem_ClientPostAdvanceTick"
    payload = {
