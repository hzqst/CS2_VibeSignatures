...
Programmatically determines IGameSystem_ClientPauseSimulate by:
1. Reading CLoopModeGame_OnClientPauseSimulate func_va from its YAML.
2. Finding `lea rdx, sub_XXXXXXX` in that function (= GameSystem_OnClientPauseSimulate).
3. Finding the virtual call offset inside GameSystem_OnClientPauseSimulate.
...
async def preprocess_skill(
    ...
    """Resolve IGameSystem_ClientPauseSimulate from CLoopModeGame_OnClientPauseSimulate."""
    _ = skill_name
    ...
    # 1. Read CLoopModeGame_OnClientPauseSimulate YAML to get func_va
    src_path = os.path.join(
        new_binary_dir,
        f"CLoopModeGame_OnClientPauseSimulate.{platform}.yaml",
    )
    src_data = _read_yaml(src_path)
    if not isinstance(src_data, dict) or not src_data.get("func_va"):
        if debug:
            print("    Preprocess: failed to read CLoopModeGame_OnClientPauseSimulate YAML")
        return False
    ...
    # 3. Use py_eval to find GameSystem_OnClientPauseSimulate and its vfunc offset
    ...
    if not isinstance(vfunc_info, dict):
        if debug:
            print("    Preprocess: failed to determine vfunc offset from CLoopModeGame_OnClientPauseSimulate")
        return False
    ...
    # 3b. Rename GameSystem_OnClientPauseSimulate in IDA
    if game_event_addr:
        try:
            await session.call_tool(
                name="rename",
                arguments={"batch": {"func": {"addr": game_event_addr, "name": "GameSystem_OnClientPauseSimulate"}}},
            )
        except Exception:
            if debug:
                print("    Preprocess: failed to rename GameSystem_OnClientPauseSimulate (non-fatal)")
    ...
    if not isinstance(func_info, dict):
        if debug:
            print("    Preprocess: failed to query function info for IGameSystem_ClientPauseSimulate")
        return False
    ...
    target_name = "IGameSystem_ClientPauseSimulate"
    payload = {
