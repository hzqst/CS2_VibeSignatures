...
Programmatically determines IGameSystem_Disconnect by:
1. Reading CSource2Client_OnDisconnect func_va from its YAML.
2. Finding `lea rdx, sub_XXXXXXX` in that function (= GameSystem_OnDisconnect).
3. Finding the virtual call offset inside GameSystem_OnDisconnect.
...
async def preprocess_skill(
    ...
    """Resolve IGameSystem_Disconnect from CSource2Client_OnDisconnect."""
    _ = skill_name
    ...
    # 1. Read CSource2Client_OnDisconnect YAML to get func_va
    src_path = os.path.join(
        new_binary_dir,
        f"CSource2Client_OnDisconnect.{platform}.yaml",
    )
    src_data = _read_yaml(src_path)
    if not isinstance(src_data, dict) or not src_data.get("func_va"):
        if debug:
            print("    Preprocess: failed to read CSource2Client_OnDisconnect YAML")
        return False
    ...
    # 3. Use py_eval to find GameSystem_OnDisconnect and its vfunc offset
    #    GameSystem function uses tail-call: `jmp qword ptr [rax+offset]` (not `call`).
    py_code = (
    ...
    if not isinstance(vfunc_info, dict):
        if debug:
            print("    Preprocess: failed to determine vfunc offset from CSource2Client_OnDisconnect")
        return False
    ...
    # 3b. Rename GameSystem_OnDisconnect in IDA
    if game_system_addr:
        try:
            await session.call_tool(
                name="rename",
                arguments={"batch": {"func": {"addr": game_system_addr, "name": "GameSystem_OnDisconnect"}}},
            )
        except Exception:
            if debug:
                print("    Preprocess: failed to rename GameSystem_OnDisconnect (non-fatal)")
    ...
    if not isinstance(func_info, dict):
        if debug:
            print("    Preprocess: failed to query function info for IGameSystem_Disconnect")
        return False
    ...
    target_name = "IGameSystem_Disconnect"
    payload = {
