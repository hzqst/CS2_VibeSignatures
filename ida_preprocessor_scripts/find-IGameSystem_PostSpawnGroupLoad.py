...
Programmatically determines IGameSystem_PostSpawnGroupLoad by:
...
2. Platform-specific vfunc offset extraction:
   - Windows: Finding the 2nd `lea rdx, sub_XXXXXXX` (= GameEvent_PostSpawnGroupLoad callback),
     then extracting the vfunc offset from its `call/jmp [rax+offset]`.
   - Linux: Finding the 2nd `mov esi, <odd_imm>` before a dispatcher call,
...
async def preprocess_skill(
    ...
    """Resolve IGameSystem_PostSpawnGroupLoad from CSpawnGroupMgrGameSystem_SpawnGroupSpawnEntitiesInternal."""
    _ = skill_name
    ...
    #
    # Windows: The 2nd `lea rdx, sub_XXX` loads the PostSpawnGroupLoad callback;
    #          callback does `jmp/call [rax+offset]` where offset is the vfunc byte offset.
    #
    # Linux:   The 2nd `mov esi, <odd_imm>` before a dispatcher call,
    #          where vfunc_offset = imm - 1.
    py_code = (
        ...
        "if func:\n"
        "    # Try Windows pattern: find the 2nd lea rdx, sub_XXX\n"
        "    lea_rdx_targets = []\n"
        ...
        "                lea_rdx_targets.append(target)\n"
        "    if len(lea_rdx_targets) >= 2:\n"
        "        game_event_addr = lea_rdx_targets[1]\n"
        ...
        "                            })\n"
        "                            break\n"
        "    elif len(lea_rdx_targets) == 0:\n"
        "        # Linux pattern: 2nd mov esi, <odd_imm> before dispatcher call\n"
        "        dispatch_count = 0\n"
        "        last_esi_imm = None\n"
        "        for head in idautils.Heads(func.start_ea, func.end_ea):\n"
        "            mnem = idc.print_insn_mnem(head)\n"
    ...
    # 3b. Rename GameEvent_PostSpawnGroupLoad in IDA (Windows only)
    if game_event_addr:
        try:
            await session.call_tool(
                name="rename",
                arguments={"batch": {"func": {"addr": game_event_addr, "name": "GameEvent_PostSpawnGroupLoad"}}},
            )
        except Exception:
            if debug:
                print("    Preprocess: failed to rename GameEvent_PostSpawnGroupLoad (non-fatal)")
    ...
    if not isinstance(func_info, dict):
        if debug:
            print("    Preprocess: failed to query function info for IGameSystem_PostSpawnGroupLoad")
        return False
