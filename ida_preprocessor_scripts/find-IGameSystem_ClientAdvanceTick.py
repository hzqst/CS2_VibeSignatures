...
Programmatically determines IGameSystem_ClientAdvanceTick by:
1. Reading CLoopModeGame_OnClientAdvanceTick func_va from its YAML.
2. Finding `lea rdx, sub_XXXXXXX` in that function (= GameSystem_OnClientAdvanceTick).
3. Finding the virtual call offset inside GameSystem_OnClientAdvanceTick.
...
async def preprocess_skill(
    ...
    """Resolve IGameSystem_ClientAdvanceTick from CLoopModeGame_OnClientAdvanceTick."""
    _ = skill_name
    ...
    # 1. Read CLoopModeGame_OnClientAdvanceTick YAML to get func_va
    src_path = os.path.join(
        new_binary_dir,
        f"CLoopModeGame_OnClientAdvanceTick.{platform}.yaml",
    )
    src_data = _read_yaml(src_path)
    if not isinstance(src_data, dict) or not src_data.get("func_va"):
        if debug:
            print("    Preprocess: failed to read CLoopModeGame_OnClientAdvanceTick YAML")
        return False
    ...
    # 3. Use py_eval to find GameSystem_OnClientAdvanceTick and its vfunc offset
    ...
    if not isinstance(vfunc_info, dict):
        if debug:
            print("    Preprocess: failed to determine vfunc offset from CLoopModeGame_OnClientAdvanceTick")
        return False
    ...
    # 3b. Rename GameSystem_OnClientAdvanceTick in IDA
    if game_event_addr:
        try:
            await session.call_tool(
                name="rename",
                arguments={"batch": {"func": {"addr": game_event_addr, "name": "GameSystem_OnClientAdvanceTick"}}},
            )
        except Exception:
            if debug:
                print("    Preprocess: failed to rename GameSystem_OnClientAdvanceTick (non-fatal)")
    ...
    if not isinstance(func_info, dict):
        if debug:
            print("    Preprocess: failed to query function info for IGameSystem_ClientAdvanceTick")
        return False
    ...
    target_name = "IGameSystem_ClientAdvanceTick"
    payload = {
