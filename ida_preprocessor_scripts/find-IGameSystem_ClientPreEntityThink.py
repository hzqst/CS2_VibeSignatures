...
async def preprocess_skill(
    ...
    """Resolve IGameSystem_ClientPreEntityThink from CLoopModeGame_OnClientPreOutputParallelWithServer."""
    _ = skill_name
    ...
    # 1. Read CLoopModeGame_OnClientPreOutputParallelWithServer YAML to get func_va
    src_path = os.path.join(
        new_binary_dir,
        f"CLoopModeGame_OnClientPreOutputParallelWithServer.{platform}.yaml",
    )
    src_data = _read_yaml(src_path)
    if not isinstance(src_data, dict) or not src_data.get("func_va"):
        if debug:
            print("    Preprocess: failed to read CLoopModeGame_OnClientPreOutputParallelWithServer YAML")
        return False
    ...
    py_code = (
        ...
        "                        break\n"
        "            if game_system_addr:\n"
        "                gef = idaapi.get_func(game_system_addr)\n"
        "                if gef:\n"
        ...
        "                                        'game_system_addr': hex(game_system_addr),\n"
    ...
    if not isinstance(vfunc_info, dict):
        if debug:
            print("    Preprocess: failed to determine vfunc offset from CLoopModeGame_OnClientPreOutputParallelWithServer")
        return False

    internal_addr = vfunc_info.get("internal_addr")
    game_system_addr = vfunc_info.get("game_system_addr")
    ...
    # 3b. Rename CLoopModeGame_OnClientPreOutputParallelWithServerInternal in IDA
    if internal_addr:
        try:
            await session.call_tool(
                name="rename",
                arguments={"batch": {"func": {"addr": internal_addr, "name": "CLoopModeGame_OnClientPreOutputParallelWithServerInternal"}}},
            )
        except Exception:
            if debug:
                print("    Preprocess: failed to rename CLoopModeGame_OnClientPreOutputParallelWithServerInternal (non-fatal)")

    # 3c. Rename GameSystem_OnClientPreEntityThink in IDA
    if game_system_addr:
        try:
            await session.call_tool(
                name="rename",
                arguments={"batch": {"func": {"addr": game_system_addr, "name": "GameSystem_OnClientPreEntityThink"}}},
            )
        except Exception:
            if debug:
                print("    Preprocess: failed to rename GameSystem_OnClientPreEntityThink (non-fatal)")
    ...
    if not isinstance(func_info, dict):
        if debug:
            print("    Preprocess: failed to query function info for IGameSystem_ClientPreEntityThink")
        return False
