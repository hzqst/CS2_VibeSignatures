...
Strategy:
- **Phase 1 (forward-only):** Try expanding only forward (after target instruction) within the function. This keeps `offset_sig_disp = 0` for the simplest case.
- **Phase 2 (backward expansion, wildcarded):** If Phase 1 fails, prepend wildcarded instructions before the target instruction (within the same function) and re-test. The `offset_sig_disp` field records the byte distance from signature start to the target instruction.
...
- Outputs the shortest unique signature as `struct_sig` with `offset_sig_disp`.
...
def main():
    ...
    # ====================================================================
    # Phase 2: Backward expansion (prepend instructions before target_inst)
    # ====================================================================
    ...
    # At each step, test the combined signature for uniqueness.
    prepend_tokens = []   # accumulated tokens from prepended instructions
    phase2_sig = None
    ...
    for i in range(len(pre_token_groups) - 1, -1, -1):
        ...
        combined = prepend_tokens + fwd_tokens

        # Try with full forward extent first, then shrink forward if possible.
        # But we must keep at least the target instruction.
        # Test from largest forward boundary down to just the target instruction.
        for boundary in reversed(fwd_boundaries):
            ...
            if test_unique(candidate, sig_start_addr):
                phase2_sig = " ".join(candidate)
                print(json.dumps({
                    "struct_sig": phase2_sig,
                    "sig_bytes": len(candidate),
                    "struct_sig_va": hex(sig_start_addr),
                    "offset_sig_disp": phase2_prepend_bytes,
                    "struct_inst_length": target_inst_len,
                    "struct_disp_offset": disp_off + phase2_prepend_bytes,
                    "struct_disp_size": disp_size,
                    "struct_offset": hex(target_struct_offset),
                    "status": "success"
                }))
    ...
    for i in range(len(pre_fixed_groups) - 1, -1, -1):
        ...
        for boundary in reversed(fwd_boundaries):
            ...
            if test_unique(candidate, sig_start_addr):
                print(json.dumps({
                    "struct_sig": " ".join(candidate),
                    "sig_bytes": len(candidate),
                    "struct_sig_va": hex(sig_start_addr),
                    "offset_sig_disp": phase3_prepend_bytes,
                    "struct_inst_length": target_inst_len,
                    "struct_disp_offset": disp_off + phase3_prepend_bytes,
                    "struct_disp_size": disp_size,
                    "struct_offset": hex(target_struct_offset),
                    "status": "success"
                }))
...
- `struct_sig_va`: VA of signature start (may be before target instruction if backward expansion was used).
- `offset_sig_disp`: Byte displacement from signature start to the target instruction. `0` means signature starts at target instruction (forward-only). Non-zero means backward expansion was used.
- `struct_inst_length`: Length of the target instruction in bytes.
- `struct_disp_offset`: Byte position of the struct offset displacement within the **signature** (= `offset_sig_disp` + displacement position within target instruction).
- `struct_disp_size`: Displacement byte size.
- `struct_offset`: The expected struct offset used for validation.

### Example Output (Forward-only, offset_sig_disp = 0)
...
### Example Output (Backward expansion, offset_sig_disp > 0)

When the target instruction is `mov rax, [rax+3A0h]` at `0x180A6D8C6` and forward-only expansion cannot produce a unique signature, backward expansion prepends preceding instructions:

```yaml
struct_sig: "FF 50 40 48 85 C0 74 0C 48 8B 80 A0 03 00 00"
struct_sig_va: 0x180A6D8BE
offset_sig_disp: 8
struct_inst_length: 7
struct_disp_offset: 10
struct_disp_size: 4
struct_offset: 0x3A0
```

In this example:
- Signature starts 8 bytes before the target instruction (`offset_sig_disp: 8`)
- The struct offset bytes `A0 03 00 00` are at byte position 10 within the signature (`struct_disp_offset: 10` = `offset_sig_disp(8)` + displacement within target instruction `(2)`)
- Runtime: scan for `struct_sig`, then add `offset_sig_disp` to get the target instruction address
