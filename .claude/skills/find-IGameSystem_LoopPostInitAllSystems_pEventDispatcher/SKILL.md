---
name: find-IGameSystem_LoopPostInitAllSystems_pEventDispatcher
...
# Find IGameSystem_LoopPostInitAllSystems_pEventDispatcher

Locate `IGameSystem::LoopPostInitAllSystems` (function) and `IGameSystem_LoopPostInitAllSystems_pEventDispatcher` (global variable) in CS2 `server.dll` or `server.so` using IDA Pro MCP tools.
...
**ALWAYS** Use SKILL `/generate-signature-for-globalvar` to generate a robust and unique signature for the global variable.
